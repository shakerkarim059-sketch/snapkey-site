export async function createGelatoOrder(order, items) {
  ...
  const gelatoItems = items.map((item) => ({
    productUid: "flat_130x180-mm-5r_170-gsm-65lb-coated-silk_4-0_ver", // kommt später aus DB
    files: [
      {
        type: "default",
    url: `${process.env.NEXT_PUBLIC_SUPABASE_URL}/storage/v1/object/public/photos/${item.photo_id_path}`
      },
    ],
    quantity: 1,
  }));
